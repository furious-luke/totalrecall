...
docker_init_database_dir() {
    ...
    # BEGIN EDITS
    # Copy configuration file override.
...
docker_try_restore() {
    if [ ! -f "$PGDATA/postgresql.conf" ]; then
        if [ ! -f "/tmp/totalrecall.restore" ]; then
            touch /tmp/totalrecall.restore
            totalrecall restore --try --copy-pg-conf=/etc/postgresql/postgresql.conf
        fi
    fi
}
# END EDITS
